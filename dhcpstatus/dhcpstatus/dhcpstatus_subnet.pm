...
LEASE:
   while (@_) {
      if ($_[0] ne "lease") {		# ignore anything that isn't a lease.
         $aux = shift;
         while ((defined $aux) && $aux ne "}") { $aux = shift; }
         next LEASE;
      }
      shift;					# keyword "lease".
...
$fijas_conf_file = "/etc/dhcp/fijas-".$subnet->subnet_id.".conf";

my $encontre = 0;

my $display2 = Display->new;

$display2->headings("Dir ip", "Estado", "-", "-",
                      "mac", "-", "Equipo");
$display2->widths(15, 6, 10, 10, 9, 14, 9);


if (open(FILE, $fijas_conf_file)){

   while (defined(FILE) && (my $line = <FILE>) && ($encontre == 0)) {
      if (substr($line, 0, 5) eq "# Red") {  # is it a static subnet?      
         my @aux_array = split " ",$line; 
         my $red = @aux_array[2]; # obtengo red  
         my $aux = "".$subnet->ip_min;
         my @aux_array2 = split /\./,$aux;
         $aux_array2[3] = '0';
         my $red_actual = join ".",@aux_array2;

         if ($red eq $red_actual){
            $encontre = 1;         

            # RECORRER HOSTS
            while (defined(FILE) && ($line = <FILE>) && (substr($line, 0, 5) ne "# Fin")) {
               my @aux_array3 = split " ",$line;
               
               if ($aux_array3[0] eq "host") {  # is this a host?
                  $host = $aux_array3[1];
                  my $salir = 0;
                  # RECORRER CAMPOS DEL HOST
                  while (defined(FILE) && ($line = <FILE>) && $salir eq 0) {
                     $aux_index = index($line, "}");
                     if ($aux_index >= 0){  
                        $salir = 1;                      
                     }else{                          
                        my @aux_array4 = split " ",$line; 
                        #MAC
                        if ($aux_array4[0] eq "hardware"){                      
                           $mac = substr($aux_array4[2], 0,-1);
                        } 
                        #IP 
                        if ($aux_array4[0] eq "fixed-address"){                      
                            $ip = substr($aux_array4[1], 0,-1);
                        }                         
                     }   
                  }
                  my @row = ($ip,"Static","","",$mac,"",$host);      
                  $display2->row(@row);
               } 
            }
         }
...
##########################################
   ...
   $display->parm("RED", $subnet->subnet_id);
   $display->parm("M&aacute;SCARA", $subnet->netmask);
   $display->parm("Broadcast", $subnet->broadcast);
   $display->parm("GATEWAY", $subnet->router);
   $display->parm("Servidores DNS", $subnet->dns_server);
   $display->parm("Servidores WINS", $subnet->wins_server);
   $display->parm("Rango IP", $subnet->ip_range_min." - ".$subnet->ip_range_max);

   $display->headings("Dir ip", "Estado", "Inicio", "Fin",
                      "mac", "DNS", "Equipo");
   $display->widths(15, 6, 10, 10, 9, 14, 9);
   $display->static_display($display2);
