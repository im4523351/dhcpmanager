...
sub get_symbols {
   ...
   if (open(FILE, $file)){
      while (defined(FILE) && (my $line = <FILE>)) {
         if (substr($line, 0, 7) eq 'include') {  # is it an include?
            my @aux_array = split "\"",$line;
            my $aux_conf = @aux_array[1];
            if (open(AUX_FILE, $aux_conf)){
               my $static_line = <AUX_FILE>;
               if (substr($static_line, 0, 7) ne '#static') { # is it a static ips file ?
                  while (defined(AUX_FILE) && (my $line = <AUX_FILE>)) {
...
sub get_subnet {
   ...
   while ($brace_count > 0) {
      if ($$symref[0] eq "range") {		# add ip range info.
         ...
         $subnet->ip_range_min(@range[0]);
         $subnet->ip_range_max(@range[-1]);
         $subnet->ips(@range);
         next;
      }
      if ($$symref[0] eq "pool") {
