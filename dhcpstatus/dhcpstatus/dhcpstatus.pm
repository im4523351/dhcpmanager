...
LEASE:
   while (@_) {
      if (defined($_[0]) && $_[0] ne "lease") {		# ignore anything that isn't a lease.
         $aux = shift;
...
sub correlate {
   ...
   $display->title($title);
   $display->headings("Ubicacion", "Red", "M&aacute;scara", "Rango IP", "Gateway",
                      "IPs definidas", "IPs usadas", "IPs libres", "BARRA DE ESTADO");
...
#
   foreach my $subnet (@subnet) {
      ...
      my $ip_range_min = $subnet->ip_range_min;
      my $ip_range_max = $subnet->ip_range_max;
      my $used_ips = 0;
      my $free_ips = 0;
